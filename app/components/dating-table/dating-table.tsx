  ...
  return (
    <div className="mb-8 font-inter">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Your Dating Log</h2>
        <button
          onClick={handleAddRow}
          className="flex items-center space-x-2 bg-brand-pink-500 hover:bg-brand-pink-600 text-white font-medium py-2 px-4 rounded-md transition-colors shadow-sm"
        >
      ...
      <div ref={tableContainerRef} className="overflow-hidden border border-gray-200 rounded-lg shadow-sm bg-white">
              ...
              {table.getHeaderGroups().map(headerGroup => (
                <tr key={headerGroup.id} className="bg-gray-50">
                  {headerGroup.headers.map(header => (
                    <th
                      key={header.id}
                      className="relative px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider sticky top-0 bg-gray-50 whitespace-nowrap border border-gray-200 select-none"
                      style={{
      ...
      <div className="mt-4 text-xs text-gray-500">
        <p>Scroll horizontally to see all columns. Click on any field to edit. Drag the edge of column headers to resize.</p>
      </div>
      ...
      <style jsx global>{`
        ...
        .resizer:hover {
          background: rgba(255, 44, 148, 0.3);
        }
        
        .resizer[data-is-resizing="true"] {
          background: rgba(255, 44, 148, 0.6);
        }
        ...
        *[data-is-resizing="true"] * {
        ...
        /* Make sure the table uses the correct box model */
        table {
          table-layout: fixed;
          border-spacing: 0;
        }
        
        /* Ensure the table cells don't grow beyond their allocated size */
        th, td {
